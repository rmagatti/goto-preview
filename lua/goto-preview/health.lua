...
function M.check()
  ...
  if vim.fn.has "nvim-0.10" == 1 then
    health.ok "Neovim version is >= 0.10 (required)"
  else
    health.error("Neovim version is < 0.10", {
      "goto-preview requires Neovim >= 0.10",
      "Current version: " .. vim.version().major .. "." .. vim.version().minor .. "." .. vim.version().patch,
      "Please upgrade Neovim to version 0.10 or later",
    })
  end
